...
export function TraineeInfo({ttRelation}: Props) {
  ...
  const usrStatus = React.useState(ttRelation.traineeStatus ? 'Currently active' : 'Currently inactive');
  const mbrStatus = React.useState(
    ttRelation.paymentStatus ? 'Up-to-Date' : 'Pending',
  );
      ...
      <Card className="items-center justify-center border-border/0 shadow-none sm:border-border sm:shadow-sm sm:shadow-black/5">
        <View className="flex-row">
          ...
          <View className="pl-2">
            ...
            <Text className="text-2xl text-white">{usrStatus.toString()}</Text>
          </View>
        </View>
        ...
        <Text className="text-xl text-white">{mbrStatus.toString()}</Text>
        <Text className="text-xl text-white">{ttRelation?.paymentDate.toString()}</Text>
