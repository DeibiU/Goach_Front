...
export function ExerciseForm({ selectedExercise, onReload }: ExerciseFormProps) {
  ...
  return (
    <View
      className={isWeb ? 'items-center bg-black/70 py-[5%]' : 'items-center bg-black/70 py-[15%]'}
    >
      <View
        className={
          isWeb
            ? 'w-3/5 rounded-2xl shadow-[rgba(0,100,255,0.5)_-5px_-4px_10px_1px]'
            : 'rounded-2xl'
        }
      >
        <Card className="border-border/0sm:border-border bg-black">
          ...
          <CardContent className="gap-6">
            ...
            <View className="gap-1.5">
              ...
              <View className="overflow-hidden rounded-lg border border-gray-500">
                <Picker
                  ...
                  mode="dropdown"
                  onValueChange={(value) => setField('muscleGroup')(value)}
                >
