...
import { Routine, Set, SetExercise, Exercise } from '../interfaces/types';
...
export function RoutineForm({ onClose, selectedRoutine }: RoutineModalProps) {
  ...
  if (selectedRoutine) {
    useEffect(() => {
      const loadSet = async () => {
        const sets: Array<Set> = await getAllSetsInRoutine(selectedRoutine?.id);
        setRoutineSets(sets);
      };
            ...
            {[
              ...
              <View key={field.id} className="gap-1.5">
                ...
                <Input
                  ...
                  onChangeText={(text) => setForm({ ...form, [field.id]: text })}
                />
              </View>
            ))}
            {[
              ...
              <View key={field.id} className="gap-1.5">
                ...
                <Input
                  ...
                  value={(form as any)[field.id].toString()}
